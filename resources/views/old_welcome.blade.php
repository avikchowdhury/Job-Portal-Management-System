...
<div class="container">
    ...
    <div>
        <a href="{{route('alljobs')}}"> <button class="btn btn-primary btn-lg" style="width: 100%;">Browse all
                jobs</button></a>
    </div>
    <br><br>
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            <div class="card" style="width: 18rem;">
                @if($company->logo)
                <center><img src="{{asset('uploads/logo')}}/{{$company->logo}}" width="60%"></center>
                @else
                <center>  <img src="{{asset('uploads/logo/logo.jpg')}}" width="60%"></center>
                @endif
                <div class="card-body">
                    <h5 class="card-title">{{$company->cname}}</h5>
                    <p class="card-text">{{Str::limit($company->description,20)}}</p>
                    <center> <a href="{{route('company.index',[$company->id,$company->slug])}}"
                            class="btn btn-outline-primary ">Visit company</a></center>
                </div>
            </div>
        </div>
        @endforeach
